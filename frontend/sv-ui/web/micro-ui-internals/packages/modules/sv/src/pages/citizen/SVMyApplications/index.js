    ...
    <React.Fragment>
        ...
        {applicationsList?.length !== 0 && (
          <div>
            <p style={{ marginLeft: "16px", marginTop: "16px" }}>
              <span className="link">{<Link to={`/sv-ui/citizen/sv/my-applications/${t1}`}>{t("SV_LOAD_MORE_MSG")}</Link>}</span>
            </p>
          </div>
        )}
      </div>
      ...
      <p style={{ marginLeft: "16px", marginTop: "16px" }}>
        ...
        <span className="link" style={{ display: "block" }}>
          <Link to="/sv-ui/citizen/sv/apply/info">{t("SV_CLICK_TO_APPLY_NEW_APPLICATION")}</Link>
        </span>
      </p>
    </React.Fragment>
  );
