...
const ReopenComplaint = ({ match, history, parentRoute }) => {
  ...
  return (
    <Routes>
      {/* MANDATORY: component convert into element  */}
      <Route path={getRoute(match, PgrRoutes.ReasonPage)} element={<ReasonPage match={match} complaintDetails={complaintDetails} />} />
      <Route path={getRoute(match, PgrRoutes.UploadPhoto)} element={<UploadPhoto match={match} skip={true} complaintDetails={complaintDetails} />} />
      <Route path={getRoute(match, PgrRoutes.AddtionalDetails)} element={<AddtionalDetails match={match} parentRoute={parentRoute} complaintDetails={complaintDetails} />} />
